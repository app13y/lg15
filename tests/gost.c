#include <stdlib.h>
#include <string.h>
#include <stdio.h>
#include "gosthopper.h"


int testBlockEncryption(void) {
    int numberOfFailedTests_ = 0;

    const uint8_t roundKeys_[NumberOfRounds * BlockLengthInBytes] = {
            ,
    };

    uint8_t block_[BlockLengthInBytes] = {
            0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x00, 0xff, 0xee, 0xdd, 0xcc, 0xbb, 0xaa, 0x99, 0x88,
    };

    const uint8_t expectedCiphertext_[BlockLengthInBytes] = {
            0x7f, 0x67, 0x9d, 0x90, 0xbe, 0xbc, 0x24, 0x30, 0x5a, 0x46, 0x8d, 0x42, 0xb9, 0xd4, 0xed, 0xcd,
    };

    void *memory_ = malloc(BlockLengthInBytes);
    if (!memory_) {
        ++numberOfFailedTests_;
        goto cleanup;
    }

    encryptBlock(roundKeys_, block_, memory_);
    numberOfFailedTests_ += memcmp(block_, expectedCiphertext_, BlockLengthInBytes);

    cleanup:
    free(memory_);
    return numberOfFailedTests_;
}

int main(void) {
    int globalNumberOfFailedTests_ = 0, localNumberOfFailedTests_ = 0;
    printf("Running GOST selftests... ");

    if ((localNumberOfFailedTests_ = testBlockEncryption())) {
        printf("FAILED, %i errors occured.\n", localNumberOfFailedTests_);
        globalNumberOfFailedTests_ += localNumberOfFailedTests_;
    } else {
        printf("OK.\n");
    }

    return (globalNumberOfFailedTests_ != 0);
}
